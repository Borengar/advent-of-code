...
fun main() {
    ...
    for (result in matchResult) {
        clawMachines.add(ClawMachine(
            ...
            result.groups["prizeX"]!!.value.toDouble(),
            result.groups["prizeY"]!!.value.toDouble()))
    }

    var tokensUsed = 0L
    for (machine in clawMachines) {
        val buttonPresses = machine.calculateButtonPresses()
        if (buttonPresses.first % 1 == 0.toDouble() && buttonPresses.second % 1 == 0.toDouble() && buttonPresses.first <= 100 && buttonPresses.second <= 100) {
