...
class TopoMap() {
    ...
    fun getTrailheadScore(x: Int, y: Int): Int {
        val foundSummits: MutableList<Pair<Int, Int>> = mutableListOf()
        ...
        while (true) {
            ...
            // Summit reached
            if (currentHeight == 9) {
                // Increment score if this summit wasn't reached before
                if (!foundSummits.any { it.first == posX && it.second == posY }) {
                    score++
                    foundSummits.add(Pair(posX, posY))
                }
